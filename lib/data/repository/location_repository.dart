...
class LocationRepository {
  // 통신의 기본은 비동기
  Future<List<Location>> searchMaps(String query) async {
    // searchMaps 함수 호출할 때 query 넘겨받을 수 있게 파라미터 추가
    final client = Client(); // http 사용법은 우선 클라이언트 객체 생성
    final response = await client.get(
        // 객체를 생성했다면 클라이언트 get 메서드 안에
        ...
        headers: {
          // headers 파라미터에 Map 타입으로 발급받은 API 넣어주기
          'X-Naver-Client-Id': ,
    ...
    if (response.statusCode == 200) {
      ...
      final items =
          List.from(map['items']); // List에서 Map을 호출하게 되면 return 타입이 Iterable이 됨
